...
class ChannelOperationsTest(unittest.TestCase):
    ...
    def test_list_channels(self):
        ...
        stubbed_xmlrpm_call = MagicMock(
            side_effect=[False, read_data_from_fixture("list_channels_simplified.data")]
        )
        # pylint: disable-next=protected-access
        self.mgr_sync._execute_xmlrpc_method = stubbed_xmlrpm_call
        with ConsoleRecorder() as recorder:
...
[I] SLES10-SP4-Pool for x86_64 SUSE Linux Enterprise Server 10 SP4 x86_64 [sles10-sp4-pool-x86_64]
    ...
    def test_list_channels_interactive(self):
        ...
        stubbed_xmlrpm_call = MagicMock(
            side_effect=[read_data_from_fixture("list_channels_simplified.data")]
        )
        # pylint: disable-next=protected-access
        self.mgr_sync._execute_xmlrpc_method = stubbed_xmlrpm_call
        available_channels = []
  ...
  1) [I] SLES10-SP4-Pool for x86_64 SUSE Linux Enterprise Server 10 SP4 x86_64 [sles10-sp4-pool-x86_64]
    ...
    def test_add_available_base_channel_with_mirror(self):
        ...
        expected_xmlrpc_calls = [
            call(self.mgr_sync.conn.sync.hub, "isISSPeripheral", self.fake_auth_token),
            # pylint: disable-next=protected-access
            call(
...
- res4-es-i386"""
    ...
    def test_add_unavailable_base_channel(self):
        ...
        # pylint: disable-next=protected-access
        self.mgr_sync._execute_xmlrpc_method = MagicMock(return_value=False)
...
'res4-es-i386' has not been added"""
    ...
    def test_add_channels_interactive(self):
        ...
        with patch("spacewalk.susemanager.mgr_sync.mgr_sync.cli_ask") as mock:
            ...
            # pylint: disable-next=protected-access
            self.mgr_sync._list_channels.assert_called_with(
                expand=False,
                filter=None,
                no_optionals=False,
                show_interactive_numbers=True,
    ...
    def test_sync_channels_interactive(self):
        ...
        stubbed_xmlrpm_call = MagicMock(
            side_effect=[
                False,
                read_data_from_fixture("list_channels_simplified.data"),
                [],
            ]
        )
        # pylint: disable-next=protected-access
    ...
    def test_add_channel_no_sync_packages(self):
        """Test adding a channel with --no-sync-packages option"""
        channel_label = "rhel-i386-as-4"
        channel_id = 101
        org_id = 123

        # pylint: disable-next=consider-using-f-string
        options = get_options(f"add channel {channel_label} --no-sync-packages".split())
