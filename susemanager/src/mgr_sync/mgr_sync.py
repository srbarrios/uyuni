...
class MgrSync(object):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self):
        ...
        url = "{0}://{1}:{2}{3}".format(
            scheme, self.config.host, self.config.port, self.config.uri
        )
        ...
        if "list_target" in vars(options):
            ...
            if "channel" in options.add_target:
                self._add_channels(
                    ...
                    no_sync_packages=options.no_sync_packages,
                )
    ...
    def _add_channels(
        ...
        if not no_sync:
            self._schedule_channel_reposync(
                ...
                no_sync_packages=no_sync_packages,
            )

    def _schedule_channel_reposync(
        self, channels, current_channels=None, no_sync_packages=False
    ):
            ...
            if no_sync_packages:
                ...
                for label in channels:
                    if label in current_channels:
                        ...
                        self.log.warn(f"Could not resolve ID for channel {label}")
