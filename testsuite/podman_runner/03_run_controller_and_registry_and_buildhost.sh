...
# Container hostnames (only for container startup)
AUTH_REGISTRY_NAME="authregistry.lab"
NO_AUTH_REGISTRY_NAME="noauthregistry.lab"
REGISTRY_PORT=":5000"
...
export PXEBOOT_IMAGE=sles15sp4
export TAGS="\"not @flaky\""
EOF

$PODMAN_CMD exec controller bash --login -c 'source /root/.bashrc'
$PODMAN_CMD run -d --network network --name ${AUTH_REGISTRY_NAME} -h ${AUTH_REGISTRY_NAME} -e AUTH_REGISTRY=${AUTH_REGISTRY_NAME} -e AUTH_REGISTRY_USER=${AUTH_REGISTRY_USER} -e AUTH_REGISTRY_PASSWD=${AUTH_REGISTRY_PASSWD} -p 5001:5000 docker.io/library/registry:2
$PODMAN_CMD run -d --network network --name ${NO_AUTH_REGISTRY_NAME} -h ${NO_AUTH_REGISTRY_NAME} -e NO_AUTH_REGISTRY=${NO_AUTH_REGISTRY_NAME} -e REGISTRY_HTTP_ADDR="0.0.0.0:5000" -p 5002:5000 docker.io/library/registry:2

$PODMAN_CMD run --privileged -d --network network -v ${src_dir}/testsuite:/testsuite -v /tmp/testing/buildhost_product_uuid:/sys/class/dmi/id/product_uuid -v /tmp/testing:/tmp -v ${src_dir}/testsuite/podman_runner/salt-minion-entry-point.sh:/salt-minion-entry-point.sh --volume /run/dbus/system_bus_socket:/run/dbus/system_bus_socket:ro -v /var/run/docker.sock:/var/run/docker.sock --name buildhost -h buildhost ghcr.io/$UYUNI_PROJECT/uyuni/ci-buildhost:$UYUNI_VERSION bash -c "/salt-minion-entry-point.sh server 1-SUSE-KEY-x86_64"
$PODMAN_CMD exec -d buildhost dockerd
...
NO_AUTH_CONTAINER_STATUS=$($PODMAN_CMD inspect -f '{{.State.Running}}' "${NO_AUTH_REGISTRY_NAME}" 2>/dev/null || echo "false")

if [ "${NO_AUTH_CONTAINER_STATUS}" != "true" ]; then
    echo "ERROR: The No-Auth Registry container (${NO_AUTH_REGISTRY_NAME}) is not running!"
    echo "--- ${NO_AUTH_REGISTRY_NAME} Logs ---"
    $PODMAN_CMD logs "${NO_AUTH_REGISTRY_NAME}" || true
    echo "---------------------------"
    exit 1
fi
